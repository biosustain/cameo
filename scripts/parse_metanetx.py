...
def construct_universal_model(list_of_db_prefixes, reac_xref, reac_prop, chem_prop):
    ...
    for index, row in reaction_selection.iterrows():
        try:
            stoichiometry = parse_reaction(row.mnx_equation, rev_arrow='=')
        except ValueError:
            continue
        else:
            for met, coeff in stoichiometry.items():
                #dropping compartment
                met_id = met.id.split('@')[0]
                met.name = chem_prop.loc[met_id]['name']
                try:
                    met.formula = Formula(chem_prop.loc[met_id].formula)
                except:
                    logger.debug('Cannot parse formula %s. Skipping formula' % chem_prop.loc[met_id].formula)
                    continue
                try:
                    met.charge = int(chem_prop.loc[met_id].charge)
                except (ValueError, TypeError):
                    logger.debug('Cannot parse charge %s. Skipping charge' % chem_prop.loc[met_id].charge)
                    pass
                rest = chem_prop.loc[met_id].to_dict()
            ...
            reaction.name = row['reference']
            row = row.fillna("")
            rest = row.to_dict()
            reaction.annotation = dict((key, rest[key]) for key in rest if key in ('classifs', 'reference'))
            reaction.annotation['Description'] = reac_xref[reac_xref.XREF==row['reference']].Description.iloc[0]
...
if __name__ == '__main__':
    ...
    # load metanetx data
    chem_xref = read_table('../data/metanetx/chem_xref.tsv.gz', comment='#')
    chem_xref.columns = ['XREF', 'MNX_ID', 'Description']
    reac_xref = read_table('../data/metanetx/reac_xref.tsv.gz', comment='#')
    reac_xref.columns = ['XREF', 'MNX_ID', 'Description']
    reac_prop = read_table('../data/metanetx/reac_prop.tsv.gz', comment='#', index_col=0,
                          names=['mnx_equation', 'reference', 'classifs', 'is_balanced', 'is_transport'])
    chem_prop = read_table('../data/metanetx/chem_prop.tsv.gz', comment='#', index_col=0, low_memory=False,
                           names=['name', 'source', 'formula', 'charge', 'mass', 'InChI', 'InChLKey', 'SMILES'])
