...
class FluxVariabilityResult(Result):
    ...
    def plot(self, grid=None, width=None, height=None, title=None, axis_font_size=None, reaction_ids=None):
        title = "FVA Result" if title is None else title

        if reaction_ids is None:
            if len(self._data_frame) < 10:
                reaction_ids = self._data_frame.index
            else:
                reaction_ids = self._data_frame.index[:10]
        fva_data = self._data_frame.loc[reaction_ids]

        plotting.plot_flux_variability_analysis(fva_data, grid=grid, width=width, height=height, title=title,
